...
public:
  ...
  /** \brief upload (read) SDO from a slave */
  int uploadSlaveSdo(
  ...
  bool isLinkUp() const { return master_state_.link_up; }
  uint16_t slavesResponding() const { return master_state_.slaves_responding; }
  bool anySlaveOperational() const
  {
    for (const auto & s : slave_info_)
    {
      if (s.config_state.operational)
      {
        return true;
      }
    }
  ...
  /** deactivate master configuration and release process data mapping */
  void deactivate();
