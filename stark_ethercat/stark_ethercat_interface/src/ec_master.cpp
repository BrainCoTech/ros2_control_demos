...
#define EC_NEWTIMEVAL2NANO(TV) (((TV).tv_sec - 946684800ULL) * 1000000000ULL + (TV).tv_nsec)

namespace stark_ethercat_interface
{
...
void EcMaster::addSlave(uint16_t alias, uint16_t position, EcSlave * slave)
{
  ...
  slave_info.config =
    ecrt_master_slave_config(master_, alias, position, slave->vendor_id_, slave->product_id_);
  ...
  {
    printWarning(
      "Add slave. Sync size is zero for " + std::to_string(alias) + ":" + std::to_string(position));
  }
...
int EcMaster::configSlaveSdo(
  uint16_t slave_position, SdoConfigEntry sdo_config, uint32_t * abort_code)
{
  uint8_t buffer[8];
  sdo_config.buffer_write(buffer);
  int ret = ecrt_master_sdo_download(
    master_, slave_position, sdo_config.index, sdo_config.sub_index, buffer, sdo_config.data_size(),
    abort_code);
  return ret;
}

int EcMaster::uploadSlaveSdo(
  uint16_t slave_position, uint16_t sdo_index, uint8_t sdo_sub_index, uint8_t * target,
  size_t target_size, size_t * out_size, uint32_t * abort_code)
{
  size_t data_size = 0;
  int ret = ecrt_master_sdo_upload(
    master_, slave_position, sdo_index, sdo_sub_index, target, target_size, &data_size, abort_code);
  if (out_size)
  {
    *out_size = data_size;
  }
...
bool EcMaster::activate()
{
  ...
  for (auto & iter : domain_info_)
  {
    ...
    bool domain_status =
      ecrt_domain_reg_pdo_entry_list(domain_info->domain, &(domain_info->domain_regs[0]));
    if (domain_status)
    {
...
uint64_t EcMaster::elapsedCycles() { return update_counter_; }
...
void EcMaster::checkSlaveStates()
{
  for (SlaveInfo & slave : slave_info_)
  {
    ...
    if (s.al_state != slave.config_state.al_state)
    {
      ...
      printf(
        "Slave[%u:%u]: AL State 0x%02X (%s).\n", slave.alias, slave.position, s.al_state,
        state_name);

      if (s.al_state != 0x08)
      {
        // 提示使用CLI读取诊断，避免在实时循环中进行阻塞访问
        printf(
          "Slave[%u:%u]: Hint - check 'ethercat reg_read -p%u 0x0134 2' and 'ethercat upload -p%u "
          "0x1001 0x00'.\n",
          slave.alias, slave.position, slave.position, slave.position);
      }
    }
